<template>
  <div v-if="countPost > 0">
    <section id="article">
      <b-container>
        <b-row>
          <div class="col-md-6 order-2 order-sm-1 box-article">
            <div class="row">
              <b-col md="12" sm="12" class="p-0">
                <div class="card">
                  <div class="label-portal-red">
                    <span>
                      <nuxt-link
                        :to="{ path: `article/category/${setPost()[0].category_slug}`}"
                      >{{setPost()[0].category_title}}</nuxt-link>
                    </span>
                  </div>
                  <!-- <nuxt-link :to="{ path: `article/${setPost()[0].id}/${setPost()[0].slug}`}"> -->
                  ...
                  <div class="middle m-auto text-center">
                    ...
                    <nuxt-link
                      :to="{ path: `article/${setPost()[0].id}/${setPost()[0].slug}`}"
              ...
              <b-col v-for="(val, index) in setPost2()" :key="index" md="6" sm="12" class="p-0">
                <div class="card" :class="`post-${val.id}`">
                  <div class="label-portal-red">
                    <span>
                      <nuxt-link
                        :to="{ path: `/article/category/${val.category_slug}`}"
                      >{{val.category_title}}</nuxt-link>
                    </span>
                  </div>
                  <!-- <nuxt-link :to="{ path: `/article/${val.id}/${val.slug}`}"> -->
                  ...
                  <div class="middle m-auto text-center">
                    ...
                    <nuxt-link
                      :to="{ path: `/article/${val.id}/${val.slug}`}"
                      class="btn btn-sm-portal btn-danger-lightred"
                    >Detail</nuxt-link>
                  </div>
                  <!-- </nuxt-link> -->
              ...
              <div class="button-article m-auto p-t-20 tablet">
                <nuxt-link to="/article" class="btn btn-portal btn-danger-lightred">Lihat Lainnya >></nuxt-link>
              </div>
            </div>
          </div>
