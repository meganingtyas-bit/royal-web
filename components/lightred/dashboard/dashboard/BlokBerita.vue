  ...
  <div>
    <div class="box-title fs-20 p-b-10">
      <span class="font-weight-bold text-muted">
        <img src="/assets/themes/lightred/images/subtitle_red.png" alt /> Berita Terkini
      </span>
    </div>
    <div class="box-dashboard-mitra" v-if="posts.length > 0">
      <b-card v-for="(post, index) in posts" :key="index">
        <nuxt-link :to="{ path: `/news/${post.id}/${post.slug}`}">
          <b-media>
      ...
      <nuxt-link to="/dashboard/news" class="text-red-portal" style="text-decoration:none;">
        <b-card>
    ...
    <div class="box-default pl-3" v-else>
      <h5 class="op-0-4 mt-2">"Tidak Ada Berita"</h5>
    </div>
  </div>
</template>
<script>
